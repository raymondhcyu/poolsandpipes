...
int	main()
{
	...
	printf("Child Process 2 Opening Pipeline.....\n");
	CPipe	p2("MyPipe2", 1024);		// Create a pipe 'p2' with the name "MyPipe"

	p2.Read(&i, sizeof(i));				// Read the int 'i' from the pipe
	cout << "Child Process 2 read integer = " << i << " from Pipeline.....\n";
	...
	cout << "Child Process 2 read string = " << name << ", from Pipeline.....\n";

	p2.Read(&mystruct, sizeof(mystruct));	// Read the structure from the pipeline
	cout << "Child Process 2 read [" << mystruct.x << "," << mystruct.y << "] from Pipeline.....\n";
